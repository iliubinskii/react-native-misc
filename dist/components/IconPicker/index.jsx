...
exports.default = (0, react_misc_1.memo)("IconPicker", ({ getIcon, names, onClose = typescript_misc_1.fn.noop, onSave, selectedName }) => {
    ...
    const initialScrollIndex = React.useMemo(() => typescript_misc_1.is.not.empty(selectedName)
    ...
    const renderItem = React.useCallback(({ item: name }) => (<Item_1.default getIcon={getIcon} name={name} onSelect={setLocalSelectedName} selected={name === localSelectedName}/>), [getIcon, localSelectedName, setLocalSelectedName]);
    ...
    const placeholder = React.useCallback(() => (<>
          {typescript_misc_1.a
            ...
            .map((row, key) => (<common_components_1.Row key={
    ...
    return (<common_components_1.Card actions={<>
        ...
        <common_components_1.Group.Sections>
          ...
          {filteredNames.length ? (<react_native_1.View style={{ height, width }}>
