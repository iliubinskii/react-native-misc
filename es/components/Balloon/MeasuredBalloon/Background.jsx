...
export default memo("Background", ({ height, horizontalPosition, verticalPosition, width }) => {
    ...
    const d = `
      ...
      Z
    `;
    return (<Svg height={height + triangleHeight + 2 * svgPadding} style={{ position: Position.absolute }} width={width + 2 * svgPadding}>
        <G transform={transform}>
          {shadow.map(n => (<Path d={d} key={n} stroke={colors.svgShadow} strokeWidth={strokeWidth + n}/>))}
          <Path d={d} fill={colors.surface} stroke={colors.outline} strokeWidth={strokeWidth}/>
        </G>
      </Svg>);
});
...
const { roundnessFactor, shadow, strokeWidth, svgPadding, triangleHeight, triangleOffset, triangleWidth } = consts.Balloon.MeasuredBalloon.Background;
