...
export default memo(
  ...
  ({
    children,
    failOnSwipeEnd = false,
    failOnSwipeStart = false,
    onSwipeEnd = fn.noop,
    onSwipeStart = fn.noop,
    resetKey,
    threshold = 0
  }: Props) => {
    ...
    const initialTouch = useSharedValue<TouchData | undefined>(undefined);
    ...
    const onEnd = React.useCallback(
      ({ velocityX }: GestureUpdateEvent<PanGestureHandlerEventPayload>) => {
        if (velocityX > velocityThreshold)
          ...
          if (failOnSwipeStart)
            springAnimation(pan, 0, velocityX, (finished = false) => {
              if (finished) onSwipeStart();
            });
          else
            velocityAnimation(panOpacity, -1, velocityX, (finished = false) => {
              if (finished) onSwipeStart();
            });
        else springAnimation(pan, 0, velocityX);
      },
      [
        failOnSwipeEnd,
        failOnSwipeStart,
