...
export default memo(
  "Hand",
  ({ active, index, indexFrom, length: handLength, width1, width2 }: Props) => {
    const { colors } = useThemeExtended();

    const circleProps = useAnimatedProps((): React.ComponentProps<
      typeof Circle
    > => {
    ...
    const circleFromProps = useAnimatedProps((): React.ComponentProps<
      typeof Circle
    > => {
    ...
    const handProps = useAnimatedProps((): React.ComponentProps<
      typeof Path
    > => {
      const angle = index.value * sector;
    ...
    const rangeProps = useAnimatedProps((): React.ComponentProps<
      typeof Path
    > => {
      if (
        active &&
        ...
        const index2 =
          index.value > indexFrom.value ? index.value : index.value + 12;
    ...
    return (
      <Svg
        height={size}
        style={{ position: Position.absolute, zIndex: active ? 1 : 0 }}
        width={size}
      >
        <G transform={`translate(${0.5 * size},${0.5 * size})`}>
          <AnimatedPath
            animatedProps={rangeProps}
            fill={Color.none}
            stroke={colors.calendar.selectionInterval}
          />
          <AnimatedCircle
          ...
          />
          <AnimatedCircle
            animatedProps={circleFromProps}
            fill={colors.calendar.selection}
          />
          <AnimatedPath
            animatedProps={handProps}
            fill={active ? colors.secondLine : colors.fadeMore}
          />
        </G>
      </Svg>
    );
