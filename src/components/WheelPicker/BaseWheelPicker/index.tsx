...
export default memo("BaseWheelPicker", function BaseWheelPicker<
  T extends NumStr
>({ color, customRef, disabled, largeSwipeSize, largeSwipeStopInterval, onChange, onOverflow, onScrollEnd = fn.noop, onScrollStart = fn.noop, options, smallSwipeSize, smallSwipeStopInterval, tick, value }: Props<T>) {
  ...
  const getStyle = React.useCallback((): StyleProp<ViewStyle> => {
    return {
      transform: [
