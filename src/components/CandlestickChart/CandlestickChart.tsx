...
export default memo(
  ...
  ({
    ...
    const initialRange = React.useMemo(
      (): Range =>
        enabled ? [initialRangeFrom, initialRangeTo] : [0, minSliceLength],
      [enabled, initialRangeFrom, initialRangeTo, minSliceLength]
    );
    ...
    const normalize = React.useCallback(
      (value: number) =>
        (value - verticalRangeFrom) / (verticalRangeTo - verticalRangeFrom),
      [verticalRangeFrom, verticalRangeTo]
    );
      ...
      () =>
        Gesture.Race(
            ...
            Gesture.Pan()
              ...
              .onUpdate(({ numberOfPointers, translationX }) => {
                if (numberOfPointers <= 1) {
                  ...
                  const translationXDelta =
                    translationX - panTranslationX.value;
                  ...
                  const average = worklets.limit(
                    prevAverage - translationXDelta / barWidth,
                    0.5 * prevSize,
                    data.length - 0.5 * prevSize
                  );
          ...
          Gesture.Tap().onEnd(({ x }) => {
            ...
            if (index >= range[0] && index < range[1]) runOnJS(onSelect)(index);
          })
        ),
      [
        data.length,
    ...
    return (
      <GestureDetector gesture={gesture}>
        <BaseChart
          ...
          range0={range[0]}
          range1={range[1]}
          verticalLabels={normalizedVerticalLabels}
          width={width}
          {...props}
        />
      </GestureDetector>
...
export interface Props
  extends Omit<React.ComponentProps<typeof BaseChart>, "range0" | "range1"> {
